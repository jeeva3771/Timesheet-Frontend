...
// import { readTimesheets, readUserNameAndRole, readProjectName, readTimeSheetDocumentById } from '../Api.js'
...
// 	const handleDocument = (blob, contentType, timesheetId) => {
// 		// Create a URL for the blob
// 		const objectUrl = URL.createObjectURL(blob);
		
// 		// Store both the blob URL and content type for each document
...
// 						const response = await fetch(`${apiUrl}/api/timesheets/documentimage/${timesheet.timesheetId}/?t=${Date.now()}`, { 
// 							method: 'GET',
// 							headers: myHeaders,
// 							credentials: 'include'
// 						});
...
// 						if (!response.ok) {
// 							console.error(`Failed to fetch document for timesheet ${timesheet.timesheetId}: ${response.status}`);
// 							return;
// 						}
...
// 					} catch (err) {
// 						console.error(`Error fetching document for timesheet ${timesheet.timesheetId}:`, err);
// 					}
// 				}
// 			}));
// 		} catch (error) {
...
// 								// For text files, you could display them in a pre tag if needed
// 								return (
// 									<iframe
// 										src={fileData.url}
// 										title="Text Viewer"
...
// 								return (
// 									<div>
// 										<p>This file type ({contentType}) cannot be displayed directly.</p>
...
// npm install xlsx file-saver docx-preview --save
import React, { useState, useEffect } from 'react'
import { PageBreadcrumb } from '@/components'
import styles from '../App.module.css'
import { useAuthContext } from '@/context'
...
const ReadTimeSheetList = () => {
  ...
  // Parse Excel files - simplified for specific formats
  const parseExcelFile = async (blob) => {
	...
	return (
		<>
			...
			{/* Document Viewer Modal - Simplified for specific formats */}
			<Modal show={showModal} onHide={() => setShowModal(false)} fullscreen>
				<Modal.Header closeButton>
					<Modal.Title>Document View</Modal.Title>
				</Modal.Header>
